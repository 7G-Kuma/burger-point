...
- **Incidencias/quejas en el cierre** (pedido aparte, mismo día): tarjeta nueva "🗒️ Incidencias y quejas" al final del cierre de caja con lo que haya en la tabla `incidencias` durante el rango — tipo, descripción, a qué pedido corresponde, quién la reportó y cuándo (hoy solo `cocina.html` las carga, tipo `error_cocina` u `otro`, pero cualquier página que registre incidencias a futuro aparece acá igual sin tocar este código). Si no hay ninguna, un estado positivo explícito en vez de dejar la sección vacía. El resumen escrito de arriba también menciona la cantidad si hay alguna. Reutiliza el permiso `reportes` que ya tenían propietario y encargado — el usuario pidió explícitamente que ambos roles lo vean porque los dos dirigen personal (caja/cocina/reparto) y necesitan esa visibilidad para gestionar, así que no hizo falta ningún cambio de permisos.

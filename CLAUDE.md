...
El usuario pidió una tanda nueva de mejoras en un solo mensaje largo; se separó en 5 frentes independientes y se acordó ir de a uno. **Hechos**: pulido móvil, separar el menú digital del login de staff (ambos arriba). **Faltan**:
...
2. **Personalización de productos (aderezos/extras)**: hoy solo existe el campo libre `observaciones` en el pedido. El pedido es sumar opciones estructuradas por producto (extra huevo, extra papas, sin cebolla, etc.), decidido que **algunos tienen costo y otros no** — falta definir con el usuario qué extras concretos quiere y sus precios antes de cargar los datos. Probablemente necesita una tabla nueva (algo como `extras` + `producto_extras`, o `extras` genéricos aplicables a cualquier producto) y tocar `pedido_items` para guardar qué extras eligió cada item.

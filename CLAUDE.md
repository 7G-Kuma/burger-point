...
Las 6 fases del plan original (configuración, login+roles, panel, pedidos/caja, cocina+reparto, cobros+deploy) están completas y en producción desde el 2026-09-03. Desde entonces se agregó bastante más de lo planeado originalmente — stock visible, rediseño visual + logo, seguimiento del cliente, registro/historial admin, factura numerada persistente, inventario con ingresos, menú digital de autoservicio con auto-notificación a caja, contenido real (descripciones + fotos) del menú, un rol de encargado real con permisos por sección y notificación de actividad al propietario, precios editables + promociones con descuento automático, y un pulido general de la experiencia móvil. Cada uno tiene su propia sección fechada más abajo, en orden cronológico — es la forma más confiable de saber qué existe y por qué.

**Para retomar el trabajo**: el usuario pidió una ronda nueva de mejoras el 2026-09-10 (ver "Pendiente — ronda de mejoras 2026-09-10" al final del documento) — el pulido móvil (primer ítem) ya está hecho, quedan: cuentas de cliente + fidelización, separar el menú digital del login de staff, personalización de productos (aderezos/extras) y variantes de bebida (marca + tamaño) con su stock. Re-confirmar con el usuario cuál sigue antes de arrancar cualquiera, son independientes entre sí.
...
El usuario pidió una tanda nueva de mejoras en un solo mensaje largo; se separó en 5 frentes independientes y se acordó ir de a uno. **Hecho**: pulido móvil (arriba). **Faltan, en el orden en que probablemente convenga abordarlos** (re-preguntar prioridad, no asumir):
...
2. **Separar el menú digital del login de staff**: hoy compartir el sistema significa mandar el link a `index.html` (login de staff) con un link secundario a `menu.html`. El pedido es invertirlo: el link que se comparte con clientes es directamente `menu.html`, y el login de cliente (para lo del punto 1) vive ahí adentro, no en `index.html`. Esto probablemente se resuelve junto con el punto 1 (no tiene sentido un login de cliente en el menú sin la cuenta de cliente detrás).
3. **Personalización de productos (aderezos/extras)**: hoy solo existe el campo libre `observaciones` en el pedido. El pedido es sumar opciones estructuradas por producto (extra huevo, extra papas, sin cebolla, etc.), decidido que **algunos tienen costo y otros no** — falta definir con el usuario qué extras concretos quiere y sus precios antes de cargar los datos. Probablemente necesita una tabla nueva (algo como `extras` + `producto_extras`, o `extras` genéricos aplicables a cualquier producto) y tocar `pedido_items` para guardar qué extras eligió cada item.
4. **Variantes de bebida (marca + tamaño)**: reemplazar el producto genérico "Gaseosa" por combinaciones de marca (Coca-Cola, Coca-Cola Zero, Sprite, Fanta, Pepsi) × tamaño (500ml, 750ml, 1L), decidido que **el stock se controla por cada combinación marca+tamaño** (no un stock genérico de "gaseosas"). Implica ~15 filas nuevas en `productos` (o una tabla de variantes) y sus `insumos`/`producto_insumos` correspondientes — bastante carga de datos, conviene confirmar la lista exacta de combinaciones con el usuario antes de crear todo.
